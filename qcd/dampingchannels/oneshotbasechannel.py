...
class OneShotDampingChannel(DampingChannel):
    ...
    def run(self, backend: Union[DeviceBackend, List[DeviceBackend]],
        ...
        if isinstance(backend, list):
            return list(map(lambda one_backend:
                            OneShotExecution(self._execute_all_circuits_one_backend(one_backend, iterations, timeout)),
                            backend))
    ...
    def _execute_all_circuits_one_backend(self, backend, iterations, timeout) -> OneShotResults:
        results = []
        for circuits_one_lambda in self._circuits:
            job = execute(circuits_one_lambda, backend=backend, shots=iterations)
            ...
            final_state_reshaped = self._compute_finale_state_vector_coords_reshaped(
                final_state.get('zero_amplitude'),
            ...
            final_state_reshaped['backend'] = backend
            results.append(final_state_reshaped)
        return OneShotResults(results)

    def _process_information_from_job_one_lambda(self, job, initial_states):
        results = job.result()
        final_states = {
            "zero_amplitude": [],
            "one_amplitude": [],
        ...
        for initial_states_idx, _ in enumerate(initial_states["zero_amplitude"]):
            ...
            final_state = self.convertCountsToFinalState(counts)
            final_states["zero_amplitude"].append(final_state["zero_amplitude"])
            final_states["one_amplitude"].append(final_state["one_amplitude"])

        return final_states

    def _compute_finale_state_vector_coords_reshaped(self, amplitudes_vector, input_angles_phase,
        ...
        for index_amplitudes in range(len(amplitudes_vector)):
            Theta_i = np.arccos(amplitudes_vector[index_amplitudes])
            Theta_Bloch = 2 * Theta_i
            final_state_vector_coords_z.append(np.cos(Theta_Bloch))
    #        final_state_vector_coords_z.append(np.cos(Theta_i))
        ...
        for new_index_amplitudes in range(len(amplitudes_vector)):
            new_theta_i = np.arccos((final_state_vector_coords_z[new_index_amplitudes] - center_z) / radius)
            new_phase_i = input_angles_phase[new_index_amplitudes % n_points_phase]
        ...
        return {
            'reshaped_coords_x': np.reshape(final_state_vector_coords_x, (n_points_theta, n_points_phase)),
            'reshaped_coords_y': np.reshape(final_state_vector_coords_y, (n_points_theta, n_points_phase)),
            'reshaped_coords_z': np.reshape(final_state_vector_coords_z, (n_points_theta, n_points_phase)),
            'center': center_z
        }

    def _convert_counts_to_final_state(self, counts):
        ...
        return {
            "zero_amplitude": np.sqrt(counts_zero / total_cycles),
